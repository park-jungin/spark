...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _run_diffusion_prediction(
        ...
        for t in action_head.noise_scheduler.timesteps:
            ...
            # Keep action head and hidden states on the same device when model shards span multiple GPUs.
            if action_head is not None:
                head_param = next(action_head.parameters(), None)
                if head_param is not None and actions_hidden_states.device != head_param.device:
                    actions_hidden_states = actions_hidden_states.to(head_param.device)
