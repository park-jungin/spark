...
def make_dataset_from_rlds(
    ...
    *,
    ...
    if split not in available_splits:
        # Some RLDS datasets (e.g., many LIBERO exports) only provide "train".
        # For validation mode, gracefully fallback to "train" instead of crashing.
        if (not train) and ("train" in available_splits):
            overwatch.warning(
                f"Requested split `{split}` for dataset `{name}` but available splits are "
                f"{sorted(list(available_splits))}; falling back to `train`."
            )
            split = "train"
        else:
            raise ValueError(
                f"Unknown split `{split}` for dataset `{name}`. "
                f"Available splits: {sorted(list(available_splits))}."
            )
